...
function detect_method() {
  # Detect if QM output comes from geom_opt or single_point
  shopt -s nullglob
  local ligand_name=$1

  geom_files=( "${WDDIR}/ligand_qm/${ligand_name}/"*geom_opt* )
  sp_files=( "${WDDIR}/ligand_qm/${ligand_name}/"*single_point*)

  if (( ${#geom_files[@]} > 0 )); then
    METHOD="geom_opt"
    METALDOCK_METHOD="True"
  
  elif (( ${#sp_files[@]} > 0 )); then
    METHOD="single_point"
    METALDOCK_METHOD="False"
  else
    echo "❌ No hay QM files ni en geom_opt ni en single_point para ${ligand_name}"
    exit 1
  fi
}
...
process() {
  ...
  for conformations in *${LIGAND_NAME}*.pdb; do
    ...
    detect_method ${LIGAND_NAME}
    write_MetalDock_ini_file "$conf" "${COM[x]}" "${COM[y]}" "${COM[z]}"
    copy_qm_files ${LIGAND_NAME} "./output/QM/${METHOD}/"
