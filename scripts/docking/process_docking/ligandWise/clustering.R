...
process_data <- function(rmsd_df_path, scores_path) {
  ...
  rmsd_df <- rmsd_df |> mutate(index = row_number()) |> relocate(index, .before=1)
...
write_sdf_clusters <- function(rmsd_df, sdf_path, clusters, output_path,
  ...
  for (cluster_index in seq_along(clusters)) {
    ...
    statistics$cluster_id <- cluster_index
    statistics$ligandName <- ligand_name
  
    all_cluster_statistics <<- rbind(all_cluster_statistics, statistics)
...
# Rscript -----------------------------------------------------------------
# Arguments should be Ligand_RMSD_matrix.data path,
# docking scores (sorted) and a cutoff value
args <- commandArgs()
...
all_cluster_statistics <- data.frame(
                                     cluster_id=numeric(),
                                     N=numeric(),
                                     meanEnergy=numeric(),
                                     minEnergy=numeric(),
...
write.csv(x = all_cluster_statistics,
